...
public abstract class Player {
    ...
    public String makeLeadingFirst(Board board, String move) throws OffBoardException {
        ...
        int dirvalue = Integer.parseInt(movesplit[2]);
        Direction dir = Direction.values()[dirvalue];
        if (marbleTo(board, firsthor, firstdiai, dir) == board.getIndex(lasthor, lastdiai) || marbleTo(board,
                marbleTo(board, firsthor, firstdiai, dir), dir) == board.getIndex(lasthor, lastdiai)) {
    ...
    public void setFields(Board board, String move) throws OffBoardException {
        ...
        for (int j = 0; j < 6 && !hasThree; j++) {
            if (marbleTo(board, (ball2 = marbleTo(board, firsthor, firstdiai, Direction.values()[j])),
